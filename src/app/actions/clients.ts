...
export async function approveClient(clientId: string) {
  ...
  // Auto-create "Collect Debit Card Information" todo (7-day due)
  const draft = await prisma.clientDraft.findFirst({
    where: { resultClientId: clientId },
    select: { id: true },
  })
  if (draft) {
    const debitCardDue = new Date()
    debitCardDue.setDate(debitCardDue.getDate() + 7)
    const todo = await prisma.todo.create({
      data: {
        title: 'Collect Debit Card Information',
        description: `Collect debit card information for ${client.firstName} ${client.lastName}. Once verified, issue PayPal.`,
        issueCategory: 'Collect Debit Card Information',
        clientDraftId: draft.id,
        assignedToId: client.closerId,
        createdById: session.user.id,
        dueDate: debitCardDue,
      },
    })
    await prisma.eventLog.create({
      data: {
        eventType: 'TODO_ASSIGNED',
        description: `Auto-assigned: "Collect Debit Card Information" for ${client.firstName} ${client.lastName}`,
        userId: session.user.id,
        metadata: {
          todoId: todo.id,
          clientDraftId: draft.id,
