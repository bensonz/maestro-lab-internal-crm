...
import { Clock, AlertCircle, CheckCircle2, XCircle, Hourglass } from 'lucide-react'
import { cn } from '@/lib/utils'

export type StatusFilter = 'inProgress' | 'needsInfo' | 'pendingApproval' | 'approved' | 'rejected'
...
const statusRows: {
  ...
  },
  {
    ...
    activeClass: 'bg-destructive/20 text-destructive ring-1 ring-destructive/30',
    statKey: 'rejected',
  },
]
...
export function ClientsSummaryPanel({
  ...
  const successRate = totalResolved > 0 ? Math.round((stats.approved / totalResolved) * 100) : 0
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive ? row.activeClass : 'text-foreground hover:bg-muted/50'
              )}
            >
              <div className="flex items-center gap-2.5">
                <Icon className={cn('h-4 w-4', isActive ? '' : row.colorClass)} />
                <span className="font-medium">{row.label}</span>
              </div>
              <span className={cn('font-mono text-sm', isActive ? '' : 'text-muted-foreground')}>
                {count}
              </span>
            </button>
          )
